...
import { ProBadge, Badge } from "../../shared/ui/Status";
...
export function Gateway() {
  ...
  return <GatewayContent data={{ upstreams: query.upstreams, profiles: query.profiles, proxyCA: query.proxyCA, proxyRules: query.proxyRules }} pro={hasProLicense(query.license)}/>;
}

function GatewayContent({ data, pro }: { data: GatewayData; pro: boolean }) {
  const { t } = useI18n();
  ...
  return <div className="gateway-page">
    ...
    <div id="gateway-panel" className="gateway-stage" role="tabpanel" aria-labelledby={tab === "api" ? "gateway-tab-api" : "gateway-tab-proxy"}>{tab === "api" ? <APIGateway data={data} pro={pro}/> : <ProxyGateway data={data} pro={pro}/>}</div>
  </div>;
}

function APIGateway({ data, pro }: { data: GatewayData; pro: boolean }) {
  ...
  return <div className="gateway-workspace">
    ...
    <ProviderDialog item={editing} data={data} pro={pro} onClose={() => setEditing(null)}/>
...
function ProxyGateway({ data, pro }: { data: GatewayData; pro: boolean }) {
  const { t } = useI18n(); const { notify } = useApp(); const qc = useQueryClient();
  ...
  return <div className="gateway-workspace">
    ...
    <ProxyRuleDialog item={editing} data={data} pro={pro} onClose={() => setEditing(null)}/>
...
function ProviderDialog({ item, data, pro, onClose }: { item: Upstream | null; data: GatewayData; pro: boolean; onClose: () => void }) {
...
function ProfileDirectoryButton({ pro }: { pro: boolean }) {
  ...
  return <span className="profile-config-entry"><Button variant="ghost" className="mr-auto px-1.5" icon={<FolderOpen size={14}/>} onClick={() => pro ? void openDirectory() : notify(t("proRequired"))}>{t("customProfileConfig")}</Button><ProBadge>{t("proPlanBadge")}</ProBadge></span>;
}
