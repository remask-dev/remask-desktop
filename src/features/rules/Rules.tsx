...
import type { BootstrapData, PolicySettings, RuleConfig } from "../../shared/api/types";
...
const titleCaseLabel = (value: string) => value.replace(/[_./-]+/g, " ").trim().split(/\s+/).filter(Boolean).map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(" ");
function modelLabelRows(data: BootstrapData) {
  const active = data.models.find(model => model.active) ?? (data.activeModel ? data.models.find(model => model.id === data.activeModel?.id) : undefined);
...
export function Rules({ data }: { data: BootstrapData }) {
  ...
  const labels = modelLabelRows(data);
  const requestVersion = useRef(0);
  const persist = async (next: PolicySettings) => {
    const version = ++requestVersion.current;
    const previous = qc.getQueriesData<BootstrapData>({ queryKey: ["core"] });
    const optimistic = normalizePolicy(next);
    // 同步更新 Shell 使用的缓存，离开再回来时仍显示最新开关；不 invalidate，避免页面刷新。
    qc.setQueriesData<BootstrapData>({ queryKey: ["core"] }, current => current ? { ...current, policy: optimistic } : current);
    try {
      const saved = normalizePolicy(await coreApi.savePolicy(next));
      if (version !== requestVersion.current) return;
      setPolicy(saved);
      qc.setQueriesData<BootstrapData>({ queryKey: ["core"] }, current => current ? { ...current, policy: saved } : current);
    } catch (error) {
  ...
  return <div className="rules-page">
    <section className="entity-controls"><div className="entity-controls__heading"><span className="rules-icon"><ShieldCheck size={17}/></span><div><h2>{t("entityProtection")}</h2><p>{t("entityProtectionSub")}</p></div></div><div className="entity-toggle-grid">{labels.map(entity => { const messageKey = entityLabels[entity.value]; const raw = entity.value.trim(); const key = raw.toUpperCase(); const fallback = titleCaseLabel(raw); const label = messageKey ? t(messageKey) : entityFriendlyLabels[locale][key as keyof typeof entityFriendlyLabels.zh] || fallback || (locale === "zh" ? "未命名实体" : "Unnamed entity"); const configured = policy.entity_types.find(item => item.type === raw); const enabled = configured?.enabled !== false; return <label key={raw || "unnamed-entity"} className={enabled ? "is-enabled" : ""}><span><strong>{label}</strong><code>{raw || "UNKNOWN"}</code></span><Switch ariaLabel={`${label} · ${raw || "UNKNOWN"}`} checked={enabled} onCheckedChange={checked => patchEntity(raw || "UNKNOWN", checked)}/></label>; })}</div></section>
    <section className="custom-rules">
      ...
      <div className="rules-container">
        ...
        <footer className="rules-footer"><p>{t("deterministicRuleNote")}</p><Button variant="primary" onClick={() => void persist(policy)}>{t("saveRules")}</Button></footer>
      </div>
    </section>
  </div>;
}
