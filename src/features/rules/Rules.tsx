...
import { FREE_CUSTOM_RULE_LIMIT, hasProLicense } from "../../shared/license";
...
export function Rules() {
  ...
  return <RulesContent data={{ policy: query.policy, models: query.models.models, pro: hasProLicense(query.license) }}/>;
}

function RulesContent({ data }: { data: { policy: PolicySettings; models: ModelPackage[]; pro: boolean } }) {
  ...
  const labels = modelLabelRows(data.models);
	...
	const addRule = () => {
		if (!data.pro && policy.rules.length >= FREE_CUSTOM_RULE_LIMIT) {
			notify(t("proRequired"));
			return;
		}
		setPolicy(current => {
			if (!data.pro && current.rules.length >= FREE_CUSTOM_RULE_LIMIT) return current;
  ...
  return <div className="rules-page">
    ...
    <section className="custom-rules">
      <header className="rules-toolbar"><div><span className="rules-icon rules-icon--custom"><ListPlus size={17}/></span><div><h2 className="feature-title-with-badge">{t("customRules")}<ProBadge>{t("proPlanBadge")}</ProBadge></h2><p>{enabledRuleCount} / {customRuleCount} {t("enabled")}</p></div></div><Button icon={<Plus size={13}/>} onClick={addRule}>{t("addRule")}</Button></header>
