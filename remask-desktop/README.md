...
The same view provides quick-launch buttons for Claude Code and Codex. Before launch, the desktop client adds any missing official Anthropic, OpenAI API, and ChatGPT Codex upstream domains without overwriting custom entries. A new terminal then starts with HTTP(S)_PROXY pointing to the HTTP endpoint, ALL_PROXY pointing to the SOCKS5 endpoint, and process-local CA variables already set. This does not modify the user's shell profile or global proxy settings. On macOS, reusable launchers are written under `~/.remask/launchers` with mode `0700`.
