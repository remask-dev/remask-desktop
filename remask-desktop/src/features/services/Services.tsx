...
import type { Profile, ProxyCAStatus, Upstream } from "../../shared/api/types";
import { useApp } from "../../app/AppContext";
import { queryKeys, useServicesData } from "../../app/useCore";
...
type ServicesData = { upstreams: Upstream[]; profiles: Profile[]; proxyCA: ProxyCAStatus };
export function Services(){
  const query=useServicesData();
  if(query.isPending||!query.upstreams||!query.profiles||!query.proxyCA)return <PageState pending={query.isPending} error={query.error} onRetry={()=>void query.refetch()}/>;
  return <ServicesContent data={{upstreams:query.upstreams,profiles:query.profiles,proxyCA:query.proxyCA}}/>;
}
function ServicesContent({data}:{data:ServicesData}){
  ...
  const remove=useMutation({mutationFn:coreApi.deleteUpstream,onSuccess:()=>{setConfirm(null);setSelected("");qc.invalidateQueries({queryKey:queryKeys.upstreams});},onError:error=>notify(String(error))});
...
function ServiceDialog({item,data,onClose}:{item:Upstream|null;data:ServicesData;onClose:()=>void}){const {t}=useI18n();const qc=useQueryClient();const editing=!!item&&data.upstreams.some(x=>x.id===item.id);const [form,setForm]=useState<Upstream>(item||blankService);
